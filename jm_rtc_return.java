...
public class jm_rtc_return {
    private List<List<Integer>> all_key = new ArrayList<>();
    //所有key
    public String[] rtc_return(String mm,int l_len) {
        ...
        String[] result_return = new String[jieceng(l_len)];
        int[] nums;
        nums = num_int_(l_len);
        ...
        for(int x=0;x<all_key.size();x++){
            ...
            result = result.trim();
            result_return[count] = result;
            count++;
        }
        return result_return;
    }
    //指定key
    public String rtc_mykey_return(String mm,int[] nums){
        int len = mm.length();
